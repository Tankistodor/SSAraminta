...
public class SSGuiElementInfoRegion extends Gui {
	...
	public SSGuiElementInfoRegion(int xPos, int yPos, int width, int height, List<String> tooltipStrings, int parentWidth, int parentHeight,
			GuiContainer parentGui) {
		this.width = 200;
		this.height = 20;
		this.enabled = true;
		this.xPosition = xPos;
	...
	public void drawRegion(int par2, int par3) {
		...
		if (this.drawRegion) {
			...
			Gui.drawRect(this.xPosition, this.yPosition, this.xPosition + this.width, this.yPosition + this.height, WorldUtils.to32BitColor(100 * k, 255, 0, 0));
		}
		...
		if (this.tooltipStrings != null && !this.tooltipStrings.isEmpty() && this.withinRegion) {
			...
			//j1 += this.parentGui.getTooltipOffset(par2, par3); //FIXME: 

			this.zLevel = 300.0F;
			SSGuiElementInfoRegion.itemRenderer.zLevel = 300.0F;
			int l1 = -267386864;
			...
			this.zLevel = 0.0F;
			SSGuiElementInfoRegion.itemRenderer.zLevel = 0.0F;
		}
